  ...
  private func setupStatusBar() {
    ...
    if let button = statusItem.button {
      button.image = NSImage(
        systemSymbolName: "doc.on.clipboard", accessibilityDescription: "Marklip Launcher")
    }
    ...
    toHTMLItem = NSMenuItem(
      title: "Convert to HTML", action: #selector(runToHTML), keyEquivalent: "h")
    toHTMLItem.target = self
    menu.addItem(toHTMLItem)

    toMarkdownItem = NSMenuItem(
      title: "Convert to markdown", action: #selector(runToMarkdown), keyEquivalent: "m")
    ...
    let registerItem = NSMenuItem(
      title: "Register as Startup Item", action: #selector(registerStartupItem), keyEquivalent: "")
    registerItem.target = self
    configMenu.addItem(registerItem)

    let unregisterItem = NSMenuItem(
      title: "Unregister as Startup Item", action: #selector(unregisterStartupItem),
      keyEquivalent: "")
